...
int alternativeRule(bool isCurrentCellAlive, int countSurroundingAlive);
...
int main(int argc, char *argv[])
{
  ...
  else	
  {
	  ...
	  int *finalState = runScenario(countIterations,ruleType);	
...
int *runScenario(int countSteps,int ruleType)
{
    int *arrayLatestState = NULL;

	for (int iTime  = 0; iTime < countSteps; iTime ++)
		{
		arrayLatestState = updateSpace(iTime,ruleType);
...
int *updateSpace(int iTime,int ruleType)
{
	int *currentCells;
	int *nextCells;
	...
	for (int row = 0; row < ROW_COUNT; row++)
		for (int col = 0; col < COL_COUNT; col++)
		  {
		    // update nextCells
			updateCell(currentCells, nextCells, row, col, ruleType);	// another (less efficient, but still relatively clean) alternative would be to test here if it's an even or odd step, and to call "update cell" with different swapps first two arguments based on the outcome
		  }
...
//			
void updateCell(int *currentCells, int *nextCells, int row, int col, int ruleType)
{
	...
	if (ruleType == 0 ) {
		...
		nextCells[iCurrentCell] = alternativeRule(isCurrentCellAlive, countSurroundingAlive);
	}
...
int computeLiveness(bool isCurrentCellAlive, int countSurroundingAlive)
{
...
// alternative rule for Conway's Game of Life.
// Given the current state of a patch and the count of neighbours that are live, returns true if the patch is occupied in the next timestep, false otherwise.
// For this particular rule, a patch that is current empty becomes alive iff it is surrounded by exactly 4 live neighbors.  A patch that 
...
int alternativeRule(bool isCurrentCellAlive, int countSurroundingAlive)
{
	if (isCurrentCellAlive)
		...
		return (countSurroundingAlive == 4);
}
